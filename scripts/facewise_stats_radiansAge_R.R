...
# for plotting means
RadDist=rep(0,Rfaces)
# for plotting age effect sizes
RadDist_adr2=rep(0,Rfaces)
# sex
RadDist_sdr2=rep(0,Rfaces)
# for fdr-correcting age associations
RadDist_ap=rep(0,Rfaces)
RadDist_sp=rep(0,Rfaces)
...
for (f in 1:Rfaces){
	...
	for (s in 1:remainingSubjs){
	    subj=df$SubjID[s]
	    ResFP_R=paste0('/cbica/projects/pinesParcels/results/PWs/Proced/',subj,'/',subj,'AngDist90_R.csv')
	    # if output exists
...
# saveout means
write.csv(RadDist,'~/results/PWs/MeanRadDist_R.csv',col.names=F,row.names=F,quote=F)
# saveout dr2s and ps - still needs to be merged with results from other hemi for MC correction
saveRDS(RadDist_adr2,paste0('/cbica/projects/pinesParcels/results/PWs/RadDist_adr2_R.rds'))
saveRDS(RadDist_sdr2,'/cbica/projects/pinesParcels/results/PWs/RadDist_sdr2_R.rds')
saveRDS(RadDist_ap,paste0('/cbica/projects/pinesParcels/results/PWs/RadDist_p_R.rds'))
saveRDS(RadDist_sp,'/cbica/projects/pinesParcels/results/PWs/RadDist_sp_R.rds')
