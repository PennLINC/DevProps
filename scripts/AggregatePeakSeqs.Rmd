...
Seqs=permutations(n=8,r=4,repeats.allowed = F)
...
for (i in 1){
  # peak sequence
  pSfn=paste0('/cbica/projects/abcdfnets/results/wave_output/',subj,'/',subj,'_',tasks[i],'_PeakSeq_int')
  peakSeq=read.delim(pSfn,header=F)
  TRfn=paste0('/cbica/projects/abcdfnets/results/wave_output/',subj,'/',subj,'_',tasks[i],'_TRSeq_int')
...
ggplot(seqCountDF)+geom_point(aes(Sequence,seqCount),size=5)+theme_minimal(base_size=10)+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+geom_hline(x=50.19)
```