...
% aggregate summed vec length to observe distribution
sumL=abs(gazes_L)+abs(gels_L);
sumR=abs(gazes_R)+abs(gels_R);

% aggregate hypotenuses of vectors to observe distribution
hypL=sqrt((gazes_L.^2)+(gels_L.^2));
hypR=sqrt((gazes_R.^2)+(gels_R.^2));
% mw mask
...
% mask em
sumL_m=sumL(g_noMW_combined_L);
sumR_m=sumR(g_noMW_combined_R);
hypL_m=hypL(g_noMW_combined_L);
hypR_m=hypR(g_noMW_combined_R);

% save out for R
writetable(table(sumL_m),'sumL.csv')
writetable(table(sumR_m),'sumR.csv')
writetable(table(hypL_m),'hypL.csv')
writetable(table(hypR_m),'hypR.csv')
