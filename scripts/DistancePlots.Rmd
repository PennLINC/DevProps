...
# new approach
CorThreshs=seq(-1,1,.05)
DistThreshs=seq(0,180,4.5)
PlotBins=matrix(data=NA,nrow=41,ncol=41,dimnames=list(CorThreshs,DistThreshs))
for (i in 2:41){
...
FCED<-ggplot(df, aes(y = FC, x = Euclidean_Distance)) +
  geom_density_2d_filled()+scale_fill_gradient2(mid='gray90',high='black',limits=c(0,8600))+theme_classic()+geom_point(alpha=0)
...
FCED<-ggplot(df, aes(y = CFC, x = Euclidean_Distance)) +
  geom_hex(bins = 100)+scale_fill_gradient2(mid='gray90',high='black',limits=c(0,12600))+theme_classic()+geom_point(alpha=0)
# plot just for marginal
ggMarginal(FCED)
```
