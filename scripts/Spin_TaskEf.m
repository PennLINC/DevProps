...
% get task effect map
taskEfL = readtable(['/cbica/projects/pinesParcels/results/PWs/FDRed_ts_L.csv'],'ReadVariableNames',0);
taskEfR = readtable(['/cbica/projects/pinesParcels/results/PWs/FDRed_ts_R.csv'],'ReadVariableNames',0);
% plop into non-mw corresponding spots
dataL=zeros(1,5120);
dataL(g_noMW_combined_L)=table2array(taskEfL);
dataR=zeros(1,5120);
dataR(g_noMW_combined_R)=table2array(taskEfR);
...
outFn=strcat('/cbica/projects/pinesParcels/results/aggregated_data/TaskEf_Permuts_fs4.mat');

% write them out as a csv for spin test to deal with		
writetable(table(vertVecL),[outdir 'TaskEfs_fs4_L.csv'],'WriteVariableNames',0);
writetable(table(vertVecR),[outdir 'TaskEfs_fs4_R.csv'],'WriteVariableNames',0);
% create permutations, save out to outFn
SpinPermuFS([outdir 'TaskEfs_fs4_L.csv'], [outdir 'TaskEfs_fs4_R.csv'], 1000, outFn,'TaskEf');
