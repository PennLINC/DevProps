...
for s=1:height(Subjs)
	...
	FDFP=['/cbica/projects/hcpd/data/motion/fd/' subj{:} '/ses-V1/files/DCANBOLDProc_v4.0.0/analyses_v2/motion/task-rest_power_2014_FD_only.mat'];
	FD=load(FDFP);
	RemainingFD=FD.motion_data{21}.remaining_frame_mean_FD;
	output{s,1}=subj{:};
	output{s,2}=RemainingFD;
	% TRs filepath
	TRsFP=['/cbica/projects/hcpd/data/motMasked_contSegs/' subj{:} '/' subj{:} '_ses-baselineYear1Arm1_task-rest_ValidSegments_Trunc.txt'];
	if isfile(TRsFP)
		TRs=readtable(TRsFP);
...
% save out
writetable(table(output),'~/PWs/Subj_FD_RemTRs.csv')
