...
for T in range(len(tasks)):
	...
	if os.path.isfile(CSIfp): 
		...
		for seg in range(SegNum):
			...
			GS_troughs, _ = find_peaks(-GSinSeg, distance=10)
		...
		waveTRs=waveTRs[noPeakPwave<14,:]
		# for surviving waves
		...
		saveoutMat[4,:]=len(GS)
		saveFN=childfp + str(subj) + '_' + str(tasks[T]) + '_waveProps_80_10.csv'
		...
		saveFN_thr=childfp + str(subj) + '_' + str(tasks[T]) + '_ThreshedWaves_80_10'
