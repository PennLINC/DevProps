function mask_mw_faces_paramSweep(subj)
% Add OFD toolbox to path
addpath(genpath('/cbica/projects/pinesParcels/multiscale/scripts/derive_parcels/Toolbox'))
%%%%%%%%%%%%%%%%%%%%% Sweepa
% file names of alphas
alphaFNlist={'VeryLow','Low','High','VeryHigh'};
% used alphas (for reference)
alphaList=[0.01 0.1 10 100];

% loop over each parameter settting
for alphas=1:4;
alphaFN=alphaFNlist{alphas}
...
AngDistFP=['/cbica/projects/pinesParcels/results/PWs/Proced/' subj '/' subj '_AngDistMat4_' alphaFN '.mat'];
data=load(AngDistFP)
...
writetable(table(gAngD_L_masked),['/cbica/projects/pinesParcels/results/PWs/Proced/' subj '/' subj '_gAngDist_Masked4_L_' alphaFN '.csv']);
writetable(table(gAngD_R_masked),['/cbica/projects/pinesParcels/results/PWs/Proced/' subj '/' subj '_gAngDist_Masked4_R' alphaFN '.csv']);
end
