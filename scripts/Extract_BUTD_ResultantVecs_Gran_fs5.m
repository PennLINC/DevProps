...
Lvertex_nomw=setdiff([1:10242],mwIndVec_l);
Rvertex_nomw=setdiff([1:10242],mwIndVec_l);

% load in GPG
gpg=load('/oak/stanford/groups/leanew1/users/apines/maps/gpg_fs5.mat');
...
OutDf_L=cell(length(F_L),3);
OutDf_R=cell(length(F_R),3);
...
% time series
OutTs_L=cell(length(F_L),3,lenOpFl);
OutTs_R=cell(length(F_R),3,lenOpFl);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
LvertTS=cell(length(vx_l),3,lenOpFl);
RvertTS=cell(length(vx_r),3,lenOpFl);
...
LvertTS_num=zeros(length(vx_l),3,lenOpFl);
RvertTS_num=zeros(length(vx_r),3,lenOpFl);
for f=1:1:10242
    for m=1:1:3
        for tp=1:1:lenOpFl
            if isempty(LvertTS{f,m,tp})
                ...
                LvertTS_num(f,m,tp) = LvertTS{f,m,tp};
            end
        end
    end
end
    ...
    for m=1:1:8
        for tp=1:1:lenOpFl
            if isempty(RvertTS{f,m,tp})
                ...
                RvertTS_num(f,m,tp) = RvertTS{f,m,tp};
            end
        end
    end
end
