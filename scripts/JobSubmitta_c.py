...
while len(subjects)>0:
  ...
  if que < 8:
    ...
    if weekno > 4 or Hour < 9 or Hour > 17 :
      ...
      OpFile='/cbica/projects/pinesParcels/results/PWs/Proced/' + str(newsub) + '/' + str(newsub) + '_gAngDist_Masked4_c_L.csv'
      if not os.path.exists(OpFile):
        subprocess.run(["qsub","-l","h_vmem=13G,s_vmem=12G","qsubMatlab_c.sh",newsub])
