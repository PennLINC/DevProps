...
```{r}
ggplot(popDfThresh,aes(x=DipSDs))+geom_histogram(fill='#BC3754')+theme_classic(base_size=16)+xlim(0,22.5)+xlab('Standard Deviations of Observed from Nulls')+theme(axis.text = element_text(size=16))
...
ggplot(plotdf,aes(x=SpunDips))+geom_density(size=1.5)+geom_vline(xintercept = plotdf$ObsDip,size=2,color='#BC3754')+theme_classic(base_size=23)+ylab('')+xlab('Dip Statistic vs. Spatial Null')+guides(y="none")

# saved out at 450 x 225
