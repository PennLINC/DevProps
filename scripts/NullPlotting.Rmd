...
for (s in 1:dim(subjList)[1]){
    ...
    if (file.exists(paste0(ResFp,subj,'_SpunDips4_My.csv'))) {
      print(s)
      SpunDips<-read.csv(paste0('Y:/results/PWs/Proced/',subj,'/',subj,'_SpunDips4_VeryHigh.csv'),header=F)
...
ggplot(plotdf,aes(x=SpunDips))+geom_density(size=1.5)+geom_vline(xintercept = plotdf$ObsDip,size=2,color='#BC3754')+theme_classic(base_size=23)+ylab('')+xlab(paste0('Dip Stats:    = ',paramList[p]))+guides(y="none")+scale_x_continuous(limits =c(0,0.00823),breaks=c(0,0.004,0.008))+theme(axis.text = element_text(size=22))
