...
#nonsig counter for tnulls (p = 0.01 or greater)
nonsig=0
...
for (s in 1:dim(subjList)[1]){
    ...
    # if output exists
    if (file.exists(paste0(ResFp,subj,'_SpunDips4_My.csv'))) {
      print(s)
      SpunDips<-read.csv(paste0('Y:/results/PWs/Proced/',subj,'/',subj,'_SpunDips4_VeryLow.csv'),header=F)
...
# for temporal
#ggplot(popDfThresh,aes(x=DipSDs))+geom_histogram(fill='#BC3754')+theme_classic(base_size=20)+xlim(-6,100)+xlab('Distance from Null Distribution (SD)')+theme(axis.text = element_text(size=23))+ylab('Participants')+geom_vline(xintercept = median(popDfThresh$DipSDs),linetype='dashed',size=2)
...
#SpunDips<-read.csv(paste0('Y:/results/PWs/Proced/',subj,'/',subj,'_SpunDips4_My.csv'),header=F)
SpunDips<-read.csv(paste0('Y:/results/PWs/Proced/',subj,'/',subj,'_SpunDips4_cnct.csv'),header=F)

ObsDip<-as.numeric(SpunDips[2,1001])
SpunDips<-as.numeric(SpunDips[2,1:1000])
plotdf<-data.frame(ObsDip,SpunDips)

ggplot(plotdf,aes(x=SpunDips))+geom_density(size=1.5)+geom_vline(xintercept = plotdf$ObsDip,size=2,color='#BC3754')+theme_classic(base_size=23)+ylab('')+xlab('Dip Statistics')+guides(y="none")+theme(axis.text = element_text(size=22))
...
```{r}
# repeat for parameter sweep
# individual level, set subj manually
paramList=c('VeryLow','Low','High','VeryHigh')
for (p in 1:length(paramList)){
SpunDips<-read.csv(paste0('Y:/results/PWs/Proced/',subj,'/',subj,'_SpunDips4_',paramList[p],'.csv'),header=F)
ObsDip<-as.numeric(SpunDips[2,1001])
SpunDips<-as.numeric(SpunDips[2,1:1000])
plotdf<-data.frame(ObsDip,SpunDips)


# calculate distance from null
SpunSD=sd(SpunDips)
SpunMean=mean(SpunDips)
#print(ObsDip)
# insert into df
...
ggplot(plotdf,aes(x=SpunDips))+geom_density(size=1.5)+geom_vline(xintercept = plotdf$ObsDip,size=2,color='#BC3754')+theme_classic(base_size=23)+ylab('')+xlab(paste0('Dip Stats: ',paramList[p]))+guides(y="none")+scale_x_continuous(,breaks=c(0,0.004,0.008))+theme(axis.text = element_text(size=22))
