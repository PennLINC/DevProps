...
iDs=rep(0,dim(subjList)[1])
iPs=rep(0,dim(subjList)[1])
...
for (s in 1:dim(subjList)[1]){
    ...
    if (file.exists(paste0(ResFp,subj,'_gShapeMatrix.rds'))) {
      ...
      idipstat=readRDS(paste0(ResFp,subj,'_DipTest.rds'))
      Ds[s]=dipstat$statistic
      Ps[s]=dipstat$p.value
      iDs[s]=idipstat$statistic
      iPs[s]=idipstat$p.value
...
popDf=data.frame(Subjvec,Ds,Ps,iDs,iPs)
...
```{r}
# dev analyses
visreg(gam(PC1score~s(interview_age,k=4)+FD+sex,data=masterdf))
