...
p+geom_bar(stat='identity',color='black',aes(fill= ..x..),binwidth = 10)+xlab("Distance (Degrees)")+theme_classic(base_size = 26)+scale_x_continuous(limits=(c(5,185)))+scale_fill_gradientn("value",colors=c("blue","cyan","green","yellow","orange","red"))+ggtitle("\u2207FH Distance: Difference")+ylab('Percentage of Propagations')+theme(axis.line.x = element_line(colour = 'black', size = 2), axis.line.y = element_line(colour = 'black', size = 2),plot.title= element_text(size=30, face="bold"), axis.title = element_text(size=30, face="bold",vjust=-1),axis.text = element_text(face="bold",size=30),legend.title=element_blank(),legend.text=element_text(size=20),legend.position='none',plot.margin=margin(b=.1,t=.1,l=.1,r=2.3, unit='cm'))+ylab('count')+ylab('Percentage of Propagations')+geom_ribbon(aes(x=seq.1..18.,ymin=botPercVec*100,ymax=topPercVec*100),fill='gray80',alpha=.5)
# saved out at 850x825

```

```{r, fig.width=8,fig.height=9}
### Same as above, but for myelin map
# load in permuted old young dif
MyPermDif=read.csv('Y:/results/PWs/Old_v_young_AngDistDif_MyG_permuted.csv')
MypermDf<-data.frame(MyPermDif)

# make vectors of mean values of permuted at each bin
meanVec=colMeans(MypermDf[1:1000,])
# make vectors of 95 and 5th percentile of permuted at each bin
topPercVec=unlist(lapply(MypermDf[1:1000,1:18],quantile,prob=.95,names=F))
botPercVec=unlist(lapply(MypermDf[1:1000,1:18],quantile,prob=.05,names=F))

myplotdataf<-data.frame(seq(1,18),t(MyPermDif[1001,]))

# correct to be degrees rather than histogram bin index
myplotdataf$seq.1..18.<-myplotdataf$seq.1..18.*10

myplotdataf$meanVec=meanVec
myplotdataf$topPercVec=topPercVec
myplotdataf$botPercVec=botPercVec

plotdf<-data.frame(meanVec,topPercVec,botPercVec,seq(1:18),t(MyPermDif[1001,]))

# * 100 for percentage
p<-ggplot(myplotdataf,aes(x=seq.1..18.,y=X1001*100))

p+geom_bar(stat='identity',color='black',aes(fill= ..x..),binwidth = 10)+xlab("Distance (Degrees)")+theme_classic(base_size = 26)+scale_x_continuous(limits=(c(5,185)))+scale_fill_gradientn("value",colors=c("blue","cyan","green","yellow","orange","red"))+ggtitle("\u2207SH Distance: Difference")+ylab('Percentage of Propagations')+theme(axis.line.x = element_line(colour = 'black', size = 2), axis.line.y = element_line(colour = 'black', size = 2),plot.title= element_text(size=30, face="bold"), axis.title = element_text(size=30, face="bold",vjust=-1),axis.text = element_text(face="bold",size=30),legend.title=element_blank(),legend.text=element_text(size=20),legend.position='none',plot.margin=margin(b=.1,t=.1,l=.1,r=2.3, unit='cm'))+ylab('count')+ylab('Percentage of Propagations')+geom_ribbon(aes(x=seq.1..18.,ymin=botPercVec*100,ymax=topPercVec*100),fill='gray80',alpha=.5)
# saved out at 850x825
```
...
ggplot(BuMerge,aes(x=(interview_age)/12,y=DMN_lm_resid)) + geom_smooth(method = 'gam', formula = y~s(x,k=4), colour=('black'), fill = "#ed2224", alpha = .4,size=3) + geom_point(size=3,alpha=.5) + xlab("Age") +ylab("Default Mode Connectivity") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) + theme(axis.line.x = element_line(colour = 'black', size = 2), axis.line.y = element_line(colour = 'black', size = 2), axis.ticks.length = unit(.25, "cm"), axis.text = element_text(face="bold",size=30), axis.title = element_text(size=30, face="bold"), axis.title.y = element_text(margin = margin(t = 0, r = 47, b = 0, l = 0),vjust=-4))
