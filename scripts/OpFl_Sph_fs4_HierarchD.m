function us = OpFl_Sph_fs4_HierarchD(subj)
...
% parameter sweep, 1 already ran (default)
for kappas=1:4;
	% alpha values to sweep over
	kappaList=[4 8 16 32];
	% alpha value for this loop
	...
	for seg=1:SegNum;
		...
		for TRP=1:(SegSpan-1);
			...
			u = ofhd(N, faces_l, vx_l, segTS_l{TRP}, segTS_l{TRP+1}, h, alpha, s);
			% throw u into struct
			us.vf_left{TRPC}=u;
			% now right hemi
			u = ofhd(N, faces_r, vx_r, segTS_r{TRP}, segTS_r{TRP+1}, h, alpha, s);
			toc;
			% throw u into struct
			us.vf_right{TRPC}=u;
	...
	% saveout with alpha denoted
	kappaFNlist={'VeryCoarse','Coarse','Gran','VeryGran'};
	kappaFN=kappaFNlist{kappas}
	save(['/cbica/projects/pinesParcels/results/PWs/Proced/' subj '/' subj '_OpFl_fs4_' kappaFN '.mat'],'us','-v7.3')
end
