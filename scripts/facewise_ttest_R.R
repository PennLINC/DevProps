...
for (f in 1:Rfaces){
        ...
        for (s in 1:remainingSubjs){
            ...
            ResFP_R_c=paste0('/cbica/projects/pinesParcels/results/PWs/Proced/',subj,'/',subj,'_BUTD_R_c.csv')
            # load in dat data
            Res_c=read.csv(ResFP_R_c)
...
# saveout dif
write.csv(dif_R,'~/results/PWs/r_vs_c_dif_R.csv',col.names=F,row.names=F,quote=F)

# saveout dr2s and ps - still needs to be merged with results from other hemi for MC correction
saveRDS(T_R,paste0('/cbica/projects/pinesParcels/results/PWs/r_vs_c_t_R.rds'))
saveRDS(p_R,paste0('/cbica/projects/pinesParcels/results/PWs/r_vs_c_p_R.rds'))
