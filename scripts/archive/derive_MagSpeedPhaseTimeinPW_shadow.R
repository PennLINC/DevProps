...
for (t in 1:4){
	# Mag from magMat
	MagMatFN=paste0(childfp,subj,'_',tasks[t],'_MagMat_Thr_Sh.csv')
	...
	if (class(mtry) == "try-error") {
		...
		# save binWise Mag
		write.table(BinWiseMag,paste0(childfp,subj,'_',tasks[t],'_BinWisePWMagnitude_Sh.csv'),row.names=F,col.names=F)
		# save normative Mag
		write.table(mean(BinWiseMag),paste0(childfp,subj,'_',tasks[t],'_MeanPWMagnitude_Sh.csv'),row.names=F,col.names=F)
	}
	# get speed and time spent in waves from waveTRs; phase from waveTRs and relative delay
	waveTRsFN=paste0(childfp,subj,'_',tasks[t],'_waveTRs_Sh.csv')
	DelayMatFN=paste0(childfp,subj,'_',tasks[t],'_delayMat_Thr_Sh.csv')
	mtry <- try(read.csv(waveTRsFN, header = F),silent=T)
		...
		# save total time
                write.table(TTSW,paste0(childfp,subj,'_',tasks[t],'_TotPWTime_Sh.csv'),row.names=F,col.names=F)
		# Speed
		MedSped=median(startEnddif)
		# save median speed
		write.table(MedSped,paste0(childfp,subj,'_',tasks[t],'_MedPWDuration_Sh.csv'),row.names=F,col.names=F)
		...
		write.table(PhPGB,paste0(childfp,subj,'_',tasks[t],'_BinWisePhase_Sh.csv'),row.names=F,col.names=F)
	}
}
