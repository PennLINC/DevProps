...
HP=cifti_read('/cbica/projects/pinesParcels/data/hcp.gradients.dscalar.nii');
...
% and once for the group map (which subject doesn't matter, using last from loop)
Subjfp=['/cbica/projects/hcpd/data/SSPs/SingleParcel_1by1_fslr/' subj];
Loading_Mat = load([Subjfp '/IndividualParcel_Final_sbj1_comp17_alphaS21_1_alphaL300_vxInfo1_ard0_eta0/init_UV.mat']);
Loading_Mat = cell2mat(Loading_Mat.V);
% for all 17 networks
for i = 1:17
        HP.cdata(1:59412)=Loading_Mat(:,i);
        outputfile=['/cbica/projects/hcpd/data/SSPs/SingleParcel_1by1_fslr/Group/AtlasLoading_Network_' num2str(i) '.dscalar.nii'];
	cifti_write(HP,outputfile)
end
