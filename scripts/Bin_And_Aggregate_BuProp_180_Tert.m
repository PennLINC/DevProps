...
% load in myelin map
gpg=load('~/data/mypg_fs4.mat');
gPG_LH=gpg.gpg.gPG_LH;
gPG_RH=gpg.gpg.gPG_RH;

% calculate group My gradient on sphere
...
for s = 1:height(Subjs)
	...
	if isfile([fp '/' Subj{:} '_AngDistMat4.mat']);
		...
		Angs=load([fp '/' Subj{:} '_tertSpecific_AngDistMat4.mat']);
...
fn=['/cbica/projects/pinesParcels/results/PWs/rs_subsBuProp_Tert.csv'];
