...
# load in spun PG
subjPGfn=childfp+str(subj)+'_PG_LR_spun_32k.dscalar.nii'
PG=nb.load(subjPGfn)
PGdataObject=PG.dataobj
# extract cortical data
PGCort=PGdataObject[:,hcp.struct.cortex]
### extract percentiles ignoring 100's. need to retain real indices for time series indexing
# float allows for NaNs
PGCorFloat=np.array(PGCort[0,:],dtype=float)
PGCorFloat[PGCorFloat==100]=np.nan
...
for T in range(len(tasks)):
	...
	if os.path.isfile(CSIfp): 
		...
		for b in range(25):
			...
			gradPrctile=np.nanpercentile(PGCorFloat,(b*4))
			gradPrctile_upper=np.nanpercentile(PGCorFloat,(b*4)+4)
			# index of vertices belonging to this percentile
			boolean_of_interest=np.logical_and(PGCorFloat > gradPrctile, PGCorFloat < gradPrctile_upper)
		...
		saveFNwTR=childfp + str(subj) + '_' + str(tasks[T]) + '_waveTRs_spun.csv'
		np.savetxt(saveFNwTR,waveTRs,delimiter=",")
		saveFNDM=childfp + str(subj) + '_' + str(tasks[T]) + '_delayMat_spun.csv'
		np.savetxt(saveFNDM,delayMatrix,delimiter=",")
		# and save thresholded delay matrix to match waveTRs
		saveFNDMt=childfp + str(subj) + '_' + str(tasks[T]) + '_delayMat_Thr_spun.csv'
		np.savetxt(saveFNDMt,mostHavePeaks,delimiter=",")
		# save magnitude matrix in same fashion
		saveFNMM=childfp + str(subj) + '_' + str(tasks[T]) + '_MagMat_spun.csv'
		...
		saveFNMMt=childfp + str(subj) + '_' + str(tasks[T]) + '_MagMat_Thr_spun.csv'
		...
		saveFN_thr=childfp + str(subj) + '_' + str(tasks[T]) + '_ThreshedWaves_spun'
