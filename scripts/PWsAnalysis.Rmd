...
# just baseline
df<-subset(gordon,gordon$eventname=='baseline_year_1_arm_1')
```
...
```{r}
# big linear models
Internalizing<-lm(cbcl_scr_syn_internal_r~interview_age+sex+TRs_r+mri_info_deviceserialnumber+rsfmri_c_ngd_meanmotion+rsfmri_c_ngd_dt_ngd_dla+RestRat,data=masterdf)
...
Age<-lm(interview_age~rsfmri_c_ngd_dt_ngd_dla+sex+mri_info_deviceserialnumber+mri_info_manufacturer+rsfmri_c_ngd_meanmotion+TRs_r+numW_r,data=masterdf)

MIdPref<-lm(numTD_m~interview_age+sex+mri_info_deviceserialnumber+mri_info_manufacturer+rsfmri_c_ngd_meanmotion+tfmri_mid_all_beh_lrwpfb_nt+tfmri_mid_all_beh_srwpfb_nt,data=masterdf)
