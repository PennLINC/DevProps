...
masterdf$RestRat<-masterdf$numTD_r/masterdf$numW_r
...
masterdf2<-masterdf[masterdf$TRs_r<1000,]
colnames(masterdf2) <- make.unique(names(masterdf2))

ggplot(masterdf2,aes(numW_r,rsfmri_c_ngd_dt_ngd_dt))+geom_point(alpha=.2)+geom_smooth(method='lm')+ylab('Within-DMN FC')+xlab('Number of Waves')+theme_classic(base_size=33)
ggplot(masterdf2,aes(numW_r,rsfmri_c_ngd_dt_ngd_dla))+geom_point(alpha=.2)+geom_smooth(method='lm')+ylab('DMN-DAN FC')+xlab('Number of Waves')+theme_classic(base_size=33)

WavesMotContDMDM<-lm(rsfmri_c_ngd_dt_ngd_dt~numW_r+rsfmri_c_ngd_meanmotion+TRs_r,data=masterdf2)

WavesMotContDMDAN<-lm(rsfmri_c_ngd_dt_ngd_dla~numW_r+rsfmri_c_ngd_meanmotion+TRs_r,data=masterdf2)
...
############
# poor man's standardize variance by site
for (s in 1:length(unique(masterdf$mri_info_deviceserialnumber))){
  # for this serial number
  SN<-unique(masterdf$mri_info_deviceserialnumber)[s]
  # site df
  siteDf<-masterdf[masterdf$mri_info_deviceserialnumber==SN,]
  ### convert to units of SDs around mean
  # print starting SD
  print(sd(siteDf$numTD_r))
  # mean
  meanTD<-mean(siteDf$numTD_r)
  # subbed
  siteDfTDminMean<-siteDf$numTD_r-meanTD
  # sd
  sdTD<-sd(siteDf$numTD_r)
  # divide by
  ZscoredTd<-siteDfTDminMean/sdTD
  # add mean back
  ZscoredTdPlusMean<-ZscoredTd+meanTD
  # print ending SD
  print(sd(ZscoredTdPlusMean))
  # drop it back in df
  masterdf$numTD_r[masterdf$mri_info_deviceserialnumber==SN]<-ZscoredTdPlusMean
}
############
```

```{r}
WavesMotContDMDM<-lm(rsfmri_c_ngd_dt_ngd_dt~numW_r+rsfmri_c_ngd_meanmotion+TRs_r,data=masterdf)

WavesMotContDMDAN<-lm(rsfmri_c_ngd_dt_ngd_dla~numW_r+rsfmri_c_ngd_meanmotion+TRs_r,data=masterdf)
...
# dt-dla comparison
InternalizingFull<-lmer(rsfmri_c_ngd_dt_ngd_dla~interview_age+sex.x+(1|mri_info_deviceserialnumber:rel_family_id)+demo_comb_income_v2+rsfmri_c_ngd_meanmotion+cbcl_scr_syn_internal_r+demo_prnt_ed_v2+demo_prtnr_ed_v2+mri_info_manufacturer,data=masterdf)

InternalizingReduced<-lmer(rsfmri_c_ngd_dt_ngd_dla~interview_age+sex.x+(1|mri_info_deviceserialnumber:rel_family_id)+demo_comb_income_v2+rsfmri_c_ngd_meanmotion+demo_prnt_ed_v2+demo_prtnr_ed_v2+mri_info_manufacturer,data=masterdf)


InternalizingLM<-lm(cbcl_scr_syn_internal_r~interview_age+sex.x+mri_info_deviceserialnumber+TRs_r+rsfmri_c_ngd_meanmotion+RestRat+rsfmri_c_ngd_dt_ngd_dla+mri_info_manufacturer,data=masterdf)

CompLM<-lm(numTD_m~interview_age+numW_m+sex.x+mri_info_deviceserialnumber+TRs_m+rsfmri_c_ngd_meanmotion+nihtbx_totalcomp_uncorrected+mri_info_manufacturer,data=masterdf)
```
...
HierVec$Var1_3=as.factor(seq(1:17))

# dispersion of nets in gradient space
ggplot(data=HierVec,aes(x=Var1_1,y=Var1_2,color=Var1_3))+geom_point(size=5)+scale_colour_manual(values = rainbow(17))+theme_classic()

# translate to 1-d distance from top, euclidean distance
HierVec$GradDist=sqrt(((HierVec$Var1_1[1]-HierVec$Var1_1)^2)+((HierVec$Var1_2[1]-HierVec$Var1_2)^2))
