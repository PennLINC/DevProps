...
%initialize output array (one row for subj name, one for dmn segreg in individualized nets, one in group
DMNseg=cell(height(Subjs),3);
...
for s=1:height(Subjs)
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	%%%%%% now take measurements from group map %%%%%
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
	% this is inefficient to do every loop, but is easier to gaurantee equivalent masking
	% initialize network partition-holder
	GPartFrame=zeros((length(nomw_mask_l)+length(nomw_mask_r)),17);
	% load in network partition
	for k=1:17
	        GFPL=['/cbica/projects/hcpd/data/SSPs/SingleParcel_1by1_fslr/Group/resamp_3k_network_L' num2str(k) '.func.gii'];
	        GFPR=['/cbica/projects/hcpd/data/SSPs/SingleParcel_1by1_fslr/Group/resamp_3k_network_R' num2str(k) '.func.gii'];
	        V_Topog_L=gifti(GFPL);
	        V_Topog_R=gifti(GFPR);
	        % extract loadings from file, plant into vertices by networks matrix for this subject
	        GPartFrame(1:length(nomw_mask_l),k)=V_Topog_L.cdata(nomw_mask_l,1);
	        GPartFrame((length(nomw_mask_l)+1):(length(nomw_mask_l)+length(nomw_mask_r)),k)=V_Topog_R.cdata(nomw_mask_r,1);
	end
	% take max of each vertex to get hard parcel
	[ ~ , GhardParcel]=max(GPartFrame,[],2);

	% initialize FCmat
        GFC=zeros(17,17);
        for k=1:17
                % get vertices in this network
                VertsInNet=find(GhardParcel==k);
                % index out within network connectivity
                curNetMat=conmat(VertsInNet,VertsInNet);
                % get within-network connectivity: using TRIU to get a mask of the upper triangle and avoid the diagonal/redundancy
                wincon=mean(curNetMat(find(~triu(ones(size(curNetMat))))));
                ...
                for b=1:16
                                curOtherNet=NotKvec(b);
                                % index vertices not in up-one-level-network-N loop
                                NotKind=find(GhardParcel==curOtherNet);
                                bwMat=conmat(VertsInNet,NotKind);
                                bwcon=mean(mean(bwMat));
                                % plop it into 17x17 matrix
                                GFC(k,curOtherNet)=bwcon;
                end
        end
	notDMvec=Kvec(Kvec~=11);
        DsegGro=mean(GFC(11,notDMvec))
	% and plop group-map based into df
	DMNseg(s,3)=num2cell(DsegGro);
end
