...
% load in spun PAG vectors
sp_PGGs=load('/cbica/projects/pinesParcels/results/aggregated_data/PAGPermuts_fs4.mat');
...
sp_fp='/cbica/projects/pinesParcels/results/aggregated_data/PAPermuts_fs4.mat';
sp=load(sp_fp);
...
for S=1:1000
	...
	% test prop under 90
	sp_AngDists_und90=find(sp_AngDists < 90);
	propUnd_90=length(sp_AngDists_und90)/length(sp_AngDists(:));
	% dip test
	DTres(S)=propUnd_90;
	% save out some example distributions
	if (S<10)
		SpunAngDist_exFP=['/cbica/projects/pinesParcels/results/PWs/Proced/' subj '/' subj '_PASpunDistr' num2str(S) '.csv'];
...
Tr_AngDists_und90=find(Tr_AngDists < 90);
propUnd_90=length(Tr_AngDists_und90)/length(Tr_AngDists(:));
DTres(1001)=propUnd_90;

% save it out
SpunAngDistFP=['/cbica/projects/pinesParcels/results/PWs/Proced/' subj '/' subj '_PAGSpun_Und90.csv'];
