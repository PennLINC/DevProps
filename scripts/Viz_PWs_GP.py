...
# set plot output fp
pofp='/cbica/projects/abcdfnets/results/PWplots/' + str(subj) + '/' + str(subj)
###### import GS
...
for x in range(len(PWinst)):
	plt.axvline(x=PWinst[x]/ARCN,linewidth=.3,dashes=[1,3,1,3])
plt.matshow(CLt,cmap='gray',vmin=-2, vmax=2,extent=[0,100,0,1], aspect=15,fignum=False)

# Right Hemi
plt.subplot(3,1,3)
for x in range(len(PWinst)):
        plt.axvline(x=PWinst[x]/ARCN,linewidth=.3,dashes=[1,3,1,3])
plt.matshow(CRt,cmap='gray',vmin=-2, vmax=2,extent=[0,100,0,1], aspect=15,fignum=False)

# plot GS
plt.subplot(3,1,1)
for x in range(len(PWinst)):
        plt.axvline(x=PWinst[x],linewidth=.3,dashes=[1,3,1,3])
plt.plot(GS,c='black')
###### save plot to subj folder
plt.savefig(pofp + '_PWs_GP.png',bbox_inches='tight')
